...
(function () {
  ...
  function setInert(on) {
    // Изолируем фон, включая логотип в шапке (бургер остаётся активным для закрытия)
  ...
  if (burger && nav) {
    ...
    document.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Tab" && isOpen()) {
        // бургер (кнопка «Закрыть») — часть ловушки фокуса, хотя он вне <nav>
  ...
  function setFormInert(on) {
    ...
    Array.prototype.forEach.call(bookingForm.children, function (c) {
      ...
      // Фолбэк для браузеров без inert: прячем от таба и SR
